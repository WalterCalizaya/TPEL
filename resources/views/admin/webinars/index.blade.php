		...
		<div class="card">
			<div class="card-header">
				...
				<div class="card-header-right">
					<a href="{{ url('/admin/webinars/create') }}" class="btn waves-effect waves-light btn-primary btn-outline-primary btn-sm"> <i class="icofont icofont-ui-add" style="color:#4680ff;"></i> Agregar webinar</a>
				</div>
			</div>
					...
					<table id="tacnatelTable" class="table table-striped table-hover" style="width:100%">
						...
						<tbody>
							...
							@foreach ($eventos as $evento)
								<tr>
									<td>
										{{ Carbon::parse($evento->inicio)->format('d/m/Y') }}
										@if ($evento->vigencia==1)
											<label class="label label-success" data-toggle="tooltip" data-placement="top" data-original-title="Evento Futuro">Fut.</label>
										@elseif ($evento->vigencia==2)
											<label class="label label-warning" data-toggle="tooltip" data-placement="top" data-original-title="En menos de 30 días">30d.</label>
										@endif
									</td>
									<td>
										{{ $evento->titulo }}
										<a href="{{ url('/webinar/').'/'.$evento->slug }}" target="_blank">
									...
									<td class="text-center">
										@if ($evento->youtube_code)
											<a href="https://www.youtube.com/watch?v={{ $evento->youtube_code }}" target="_blank">
												<i class="icon feather icon-video f-w-600 f-16 m-r-15" title="Ver video"></i>
											</a>
										@endif
										@if ($evento->material)
											<a href="{{ url('/').$evento->material }}" download target="_blank">
												<i class="icon feather icon-file f-w-600 f-16 m-r-15" title="Descargar material"></i>
											</a>
										@endif
									</td>
									<td class="text-center">
									...
									<td class="text-center">
										<a href="{{ url('/admin/webinars/'.$evento->id.'/edit') }}">
