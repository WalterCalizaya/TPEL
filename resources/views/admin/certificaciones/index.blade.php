		...
		<div class="card">
			<div class="card-header">
				...
				<div class="card-header-right">
					<a href="{{ url('/admin/certificaciones/create') }}" class="btn waves-effect waves-light btn-primary btn-outline-primary btn-sm"> <i class="icofont icofont-ui-add" style="color:#4680ff;"></i> Agregar curso de certificación</a>
				</div>
			</div>
					...
					<table id="tacnatelTable" class="table table-striped table-hover" style="width:100%">
						...
						<tbody>
                            ...
                            @foreach ($eventos as $evento)
                                <tr>
                                    <td>
                                        {{ $evento->certificacion->certificacion }}
                                        <a href="{{ url('/certificaciones/').'/'.$evento->certificacion->tipocertificacion->slug.'/'.$evento->slug }}" target="_blank">
                                    ...
                                    <td>
                                        {{ $evento->distrito->provincia->region->nombre }}
                                    </td>
                                    <td>
                                        {{ Carbon::parse($evento->inicio)->format('d \d\e M') }} al {{ Carbon::parse($evento->fin)->format('d \d\e M, Y') }}
                                    ...
                                    <td class="text-center">
                                        <a href="{{ url('/admin/certificaciones/'.$evento->id.'/edit') }}">
